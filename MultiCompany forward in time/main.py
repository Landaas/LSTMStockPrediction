...
def create_sequences(dataset, look_back, shift):
                X, y = [], []
                for i in range(len(dataset) - look_back - shift):
                    X.append(dataset.iloc[i:(i + look_back)].values)
                    y.append(dataset.iloc[i + look_back + shift - 1]['Close'])
                return np.array(X), np.array(y)

def load_data_from_folder_and_train_model(folder_path, look_back, epochs, batch_size, shift):  
    for filename in os.listdir(folder_path):
        if filename.endswith('.csv') and filename:
            ...
            X, y = create_sequences(scaled_data, look_back, shift)
...
for look_back, batch_size, epochs, lstm_values, lstm_models, dropout_values, shift in product(look_back, batch_size, epochs, lstm_values, lstm_models, dropout_values, shift):
    ...
    load_data_from_folder_and_train_model(data_folder_path, look_back, epochs, batch_size, shift)
    ...
    X_test, y_test = create_sequences(scaled_data, look_back, shift)
    ...
    plt.legend()
    plot_filename = f'actual_vs_predicted_prices,{look_back},{batch_size},{epochs},{lstm_values},{lstm_models},{dropout_values},{shift}.png'
    ...
    if not os.path.isfile(results_file):
        with open(results_file, 'w') as f:
            f.write('Mean squared error,Mean Absolute Error,Root Mean Squared Error,Mean Absolute Percentage Error,R2,Look back,Batch size,Epochs,LSTM values,LSTM models,Dropout values, Shift\n')

    # Append the evaluation results to the CSV file
    with open(results_file, 'a') as f:
        f.write(f'{mse},{mae},{rmse},{mape},{r2},{look_back},{batch_size},{epochs},{lstm_values},{lstm_models},{dropout_values},{shift}\n')

    # Save the model
    model_filename = f'stock_price_prediction_model,{look_back},{batch_size},{epochs},{lstm_values},{lstm_models},{dropout_values},{shift}.keras'
