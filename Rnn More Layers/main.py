...
from tensorflow.keras.layers import LSTM, Dense, Input, Dropout, SimpleRNN
...
for look_back, batch_size, epochs, neurons, lstm_models, dropout_values in product(look_back, batch_size, epochs, neurons, lstm_models, dropout_values):
    ...
    if not os.path.isfile(results_file):
        with open(results_file, 'w') as f:
            f.write('Mean squared error,Mean Absolute Error,Root Mean Squared Error,Mean Absolute Percentage Error,R2,Look back,Batch size,Epochs,neurons,Layers,Dropout values\n')
